...
class M_wali_murid extends CI_Model {
	...
	function all_admin(){
		...
		WHERE us.m_user_id !='1' and us.is_wali_murid='1'
		";
		return $this->db->query($query);
	}

	function verifikasi_wali_murid($where){
	...
	function tambah_wali_murid($data1,$table)
		{
	...
	function hapus_wali_murid($where)
	{
		$this->db->where($where);
	...
	function edit_wali_murid($table,$where)
	{
		return $this->db->get_where($where,$table);
	}
	function update_wali_murid($where,$data,$table)
	{
		$this->db->where($where);
