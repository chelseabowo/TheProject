...
class M_kepala_sekolah extends CI_Model {
	...
	function all_kepala_sekolah(){
		...
		WHERE us.m_user_id !='1' and us.is_kepala_sekolah='1'
		";
		return $this->db->query($query);
	}

	function verifikasi_kepala_sekolah($where){
	...
	function tambah_kepala_sekolah($data1,$table)
		{
			$this->db->insert($table,$data1);
			$last_id = $this->db->insert_id();
			
			$query ="INSERT INTO d_user_role (m_user_id,m_role_id) VALUES ('$last_id','3')";
			return $this->db->query($query);
		}
	function hapus_kepala_sekolah($where)
	{
		$this->db->where($where);
	...
	function edit_kepala_sekolah($table,$where)
	{
		return $this->db->get_where($where,$table);
	}
	function update_kepala_sekolah($where,$data,$table)
	{
		$this->db->where($where);
