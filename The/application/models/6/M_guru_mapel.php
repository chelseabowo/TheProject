...
class M_guru_mapel extends CI_Model {
	...
	function tampil_guru_mapel($where){
		$query="SELECT
		url.m_user_id,
		url.d_user_role_id,
		us.user_nama,
		...
		WHERE us.is_guru_mapel='1' and sk.d_sekolah_id = (
		Select
		skl.d_sekolah_id
		From
		M_user us
	...
	function verifikasi_guru_mapel($where){
	...
	function tambah_guru_mapel($data1,$table)
		{
			$this->db->insert($table,$data1);
			$last_id = $this->db->insert_id();
			
			$query ="INSERT INTO d_user_role (m_user_id,m_role_id) VALUES ('$last_id','6')";
			return $this->db->query($query);
		}
	function hapus_guru_mapel($where)
	{
		$this->db->where($where);
	...
	function edit_guru_mapel($table,$where)
	{
