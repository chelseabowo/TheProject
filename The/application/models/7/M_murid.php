...
class M_murid extends CI_Model {
	...
	function tampil_murid($where){
		...
		LEFT JOIN d_kelas kls ON ur.d_kelas_id = kls.d_kelas_id
		WHERE ur.m_role_id = '8' AND kls.d_kelas_id = (
		Select
		kl.d_kelas_id
		From
		M_user us
		LEFT JOIN d_user_role ur ON us.m_user_id = ur.m_user_id
		LEFT JOIN d_kelas kl ON ur.d_kelas_id = kl.d_kelas_id
		WHERE ur.m_role_id = '7' AND us.m_user_id='$where[m_user_id]'
		)";
		return $this->db->query($query);
	}
