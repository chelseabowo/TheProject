...
class M_konselor extends CI_Model {

	function tampil_konselor($where){
		$query="SELECT
		url.m_user_id,
		url.d_user_role_id,
		us.user_nama,
		...
		WHERE us.is_konselor = '1' and sk.d_sekolah_id = (
		Select
		skl.d_sekolah_id
		From
		M_user us
	...
	function tambah_konselor($data1,$table)
		{
			...
			$query ="INSERT INTO d_user_role (m_user_id,m_role_id) VALUES ('$last_id','4')";
			return $this->db->query($query);
		}
	function hapus_konselor($where)
	{
		$this->db->where($where);
	...
	function edit_konselor($table,$where)
	{
		return $this->db->get_where($where,$table);
	}
	function update_konselor($where,$data,$table)
	{
		$this->db->where($where);
